...
mt76_txq_schedule_list(struct mt76_phy *phy, enum mt76_txq_id qid)
{
	...
	while (1) {
		...
		if (test_bit(MT76_RESET, &phy->state) || phy->offchannel) {
			mtk_dbg(dev, TXV, "mt76-txq-schedule-list, reset or offchannel, EBUSY\n");
			return -EBUSY;
		}
		...
		if (!txq) {
			mtk_dbg(dev, TXV, "mt76-txq-schedule-list, no ieee next txq, qid: %d, ret: %d\n",
				qid, ret);
			break;
		}
...
void mt76_txq_schedule(struct mt76_phy *phy, enum mt76_txq_id qid)
{
	...
	if (qid >= 4 || phy->offchannel) {
		mtk_dbg(phy->dev, TXV, "%mt76-txq-schedule, qid OOR: %d or offchannel.\n",
			qid);
		return;
	}
			       ...
			       struct sk_buff_head *head)
{
	...
	while ((skb = skb_peek(head)) != NULL) {
		...
		if (mt76_txq_stopped(q) || test_bit(MT76_RESET, &phy->state)) {
			mtk_dbg(phy->dev, TXV, "mt76-txq-sched-pending-wcid, wcid->idx: %d queue-stopped or in reset, qid: %d\n",
				wcid->idx, qid);
			ret = -1;
			break;
		}
